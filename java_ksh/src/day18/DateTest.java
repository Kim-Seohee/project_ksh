...
public class DateTest {
	...
	public static void main(String[] args) {
		...
		// 현재 시간을 원하는 형태로 문자열로 변경(Date => String)
		SimpleDateFormat form = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); // 어떤 패턴으로 할 지는 알아서 하면 됨
		String str = form.format(date);
		System.out.println(str);
		// 시간 표현으로 된 문자열을 시간 객체로 변경(String => Date)
		String str2 = "2021-05-21 09:50:55";
		SimpleDateFormat form2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		Date date2;
		try {
			System.out.println(str2);
			date2 = form2.parse(str2); // parseException은 실행예외가 아니므로 무조건 예외 처리를 해 줘야 함
