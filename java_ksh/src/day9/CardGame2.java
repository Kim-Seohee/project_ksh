...
public class CardGame2 {
	...
	public static String result(int []card) {
		int poker = poker(card);
		int fullHouse = fullHouse(card);
		int straight = straight(card);
		int triple = triple(card);
		int []twoPair = twoPair(card);
		int onePair = onePair(card);
		if(poker != 0) {
			...
			return fullHouse + " 풀하우스";
		} else if(straight != 0) {
			return straight + " 스트레이트";
		} else if(triple != 0) {
			return triple + " 트리플";
		} else if(twoPair != null) {
			return Arrays.toString(twoPair) + "투 페어";
		} else if(onePair != 0) {
			return onePair + " 원 페어";
		} 
