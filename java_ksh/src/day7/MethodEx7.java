...
public class MethodEx7 {
	...
	public static void main(String[] args) {
		...
		int num = 3;
		System.out.println(contains(arr, arr.length, num)); // true
		System.out.println(contains(arr, 2, num)); // false
	 ...
	 * 매개변수: 배열, 갯수, 정수 => int []arr, int count, int num
	 ...
	 * 매개변수: 배열, 랜덤한 수를 만들기 위한 범위 => int []arr int min, int max
	 * 리턴타입: 없음 => void
	...
	public static void initRandomArray(int []arr, int min, int max) {
		...
		// 예외 처리 2: min과 max가 바뀐 경우
		if(min > max) {
			int tmp = min;
			min = max;
			max = tmp;
		}
		// 예외 처리 3: 랜덤 범위의 개수가 배열의 크기보다 작은 경우
