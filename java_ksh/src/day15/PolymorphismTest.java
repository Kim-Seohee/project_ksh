...
public class PolymorphismTest {
	...
	public static void repair(Tire tire) {
		 ...
		 * ((HankookTire)tire).company: tire를 한국타이어로 타입변환 후 멤버 변수 company를 찾음
		 * */
		if(tire instanceof HankookTire) {
			HankookTire t = (HankookTire)tire;
			System.out.println(t.company + " 수리합니다.");
			System.out.println(((HankookTire)tire).company + " 수리합니다."); // .이 우선순위가 높아서 괄호를 쳐줘야 함
		}
...
class Car{
	// 인터페이스를 이용한 멤버변수(필드)의 다형성을 살펴보는 예제
