...
public class LoginInterceptor extends HandlerInterceptorAdapter {
	...
	public void postHandle(
	    ...
	    throws Exception {
		// mv.addObject("user",xxx)으로 정보를 전달한 내용이 있으면 가져오는 코드
	    ...
	    if(user != null) {
	    	...
	    	if(user.getUseCookie() != null) {
	    		...
	    		// 쿠키 유지시간(초 단위)
	    		int day = 7;
	    		int amount = 60 * 60 * 24 * day; // 하루 * day(7일) (초)
	    		loginCookie.setMaxAge(amount);
	    		response.addCookie(loginCookie);
	    		// 현재 시간에서 유지시간을 더한 날짜를 구함(로그인 시간을 기준으로 7일 뒤)
	    		Date session_limit = new Date(System.currentTimeMillis() + (1000*amount)); // 밀리초 단위
