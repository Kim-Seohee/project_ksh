...
public class UploadFileUtils {
	/* 기능: 파일 주소와 파일명과 파일이 주어지면 해당 파일주소에 업로드일을 기준으로 /년도/월/일 폴더를 만들어 해당 폴더에 파일을 업로드하는 메소드
	 * 매개변수: uploadPath - 업로드할 파일 경로, originalName - 실제 파일명, fileData- 업로드할 실제 파일 데이터가 있는 배열
	 * 리턴타입: 저장된 파일경로와 파일명으로 된 문자열
	 * 예시: uploadPath="D:\\JAVA_KSH\\uploadfiles", originalName="dog.jpg", fileData에는 dog.jpg파일이 배열로 담겨있다고 생각하면 됨
	 * */
	 ...
	 * 리턴타입: 날짜 폴더 경로 => 2021\\07\\14 (\(구분자)는 운영체제에 따라 다르다)
	 * 
	 * */
	...
	private static void makeDir(String uploadPath, String... paths) {
		// D:\\JAVA_KSH\\uploadfiles\\2021\\07\\14
		// 년월일로 된 폴더가 이미 있으면 메소드를 종료
		if(new File(uploadPath + paths[paths.length-1]).exists())
			return;
		for(String path : paths) {
			// D:\\JAVA_KSH\\uploadfiles\\2021,D:\\JAVA_KSH\\uploadfiles\\2021\\07, D:\\JAVA_KSH\\uploadfiles\\2021\\07\\14
			File dirPath = new File(uploadPath + path);
			// 파일(여기서는 폴더)가 없으면 해당 폴더를 생성한다.
			if( !dirPath.exists())
				dirPath.mkdir();
		}
	}
